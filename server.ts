...
async function startServer() {
  ...
  // ─── Gunakan PORT dari environment (Replit inject ini otomatis) ───
  ...
  const io = new Server(server, {
    ...
    // Replit pakai HTTPS proxy — pastikan WebSocket path benar
    path: "/socket.io/",
  });
  ...
  if (process.env.NODE_ENV !== "production") {
    const vite = await createViteServer({
      server: {
        middlewareMode: true,
        // Izinkan semua host agar Replit preview bisa akses
        hmr: {
          server,
          port: PORT,
        },
